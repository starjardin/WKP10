...
const editPartner = (e) => {
  ...
  if (editButton) {
    ...
    const idOfPersonToEdit = editTbleRow.dataset.id;
    editPartnerPopup(idOfPersonToEdit);
  }
};

const editPartnerPopup = (idOfPersonToEdit) => {
	// create edit popup here
  return new Promise(async function(resolve) {
    const formPopup = document.createElement("form");
    formPopup.classList.add("formPopup");
    const person = persons.find(person => person.id === idOfPersonToEdit);
    ...
    formPopup.addEventListener("submit", e => {
      ...
      const findPersonToEdit = persons.find(person => person.id = newPerson.id);
      findPersonToEdit.id = newPerson.id;
      findPersonToEdit.lastName = newPerson.lastName;
      findPersonToEdit.firstName = newPerson.firstName;
      findPersonToEdit.jobTitle = newPerson.jobTitle;
      findPersonToEdit.jobArea = newPerson.jobArea;
      findPersonToEdit.phone = newPerson.phone;
      findPersonToEdit.picture = newPerson.picture;
      displayList(persons);
...
const deletePartner = (e) => {
  ...
  return new Promise(function(resolve) {
    ...
    if (buttonDelete) {
      const deleteTbleRow = e.target.closest("tr");
      const idOfPersonTodelete = deleteTbleRow.dataset.id;
      deletePopup(idOfPersonTodelete);
    }
  })
};

const deletePopup = async (idOfPersonTodelete) => {
	// create confirmation popup here
  return new Promise(async function(resolve) {
    const deletePopup = document.createElement("div");
    deletePopup.classList.add("deletePopup");
    //see who gets clicked here
    const person = persons.find(person => person.id === idOfPersonTodelete);
    ...
    yesButton.addEventListener("click", (e) => {
      const personToDelete = persons.filter(person => person.id !== idOfPersonTodelete);
